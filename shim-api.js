...
const { fingerprintCode, scanFingerprints, removeLinesContaining } = require('./utils');
...
app.post('/api/generate', async (req, res) => {
    ...
    try {
        ...
        const fingerprint = fingerprintCode(cleanCode);
        const scanResult = await scanFingerprints(fingerprint, config);

        // Insert scan results as comment in the code
        const commentedCode = `/* STARTS autogenerated code for which\n   SCANOSS found ${scanResult} */\n\n${cleanCode}\n/* END of autogenerated code */\n`;
...
app.listen(config.server.port, config.server.host, () => {
    console.log(`AI shim API running on http://${config.server.host}:${config.server.port}`);
});